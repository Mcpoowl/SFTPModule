// This file was generated by Mendix Modeler.
//
// WARNING: Only the following code will be retained when actions are regenerated:
// - the import list
// - the code between BEGIN USER CODE and END USER CODE
// - the code between BEGIN EXTRA CODE and END EXTRA CODE
// Other code you write will be lost the next time you deploy the project.
// Special characters, e.g., é, ö, à, etc. are supported in comments.

package sftp.actions;

import sftp.helpers.FTPConnection;
import sftp.helpers.SFTPConnection;
import sftp.helpers.Action;
import sftp.proxies.FTPType;
import com.mendix.core.Core;
import com.mendix.systemwideinterfaces.core.IContext;
import com.mendix.webui.CustomJavaAction;
import com.mendix.systemwideinterfaces.core.IMendixObject;

/**
 * Tries to retrieve a document from the SFTP location
 * 
 * Document = A FileDocument entity which will be inhabited by the downloaded file
 * Connection = The user specific Connection entity
 * Configuration = The general Configuration entity
 */
public class JA_GetDocument extends CustomJavaAction<Long>
{
	private IMendixObject __DocumentParameter1;
	private system.proxies.FileDocument DocumentParameter1;
	private IMendixObject __ConnectionParameter1;
	private sftp.proxies.Connection ConnectionParameter1;
	private IMendixObject __ConfigurationParameter1;
	private sftp.proxies.Configuration ConfigurationParameter1;

	public JA_GetDocument(IContext context, IMendixObject DocumentParameter1, IMendixObject ConnectionParameter1, IMendixObject ConfigurationParameter1)
	{
		super(context);
		this.__DocumentParameter1 = DocumentParameter1;
		this.__ConnectionParameter1 = ConnectionParameter1;
		this.__ConfigurationParameter1 = ConfigurationParameter1;
	}

	@Override
	public Long executeAction() throws Exception
	{
		this.DocumentParameter1 = __DocumentParameter1 == null ? null : system.proxies.FileDocument.initialize(getContext(), __DocumentParameter1);

		this.ConnectionParameter1 = __ConnectionParameter1 == null ? null : sftp.proxies.Connection.initialize(getContext(), __ConnectionParameter1);

		this.ConfigurationParameter1 = __ConfigurationParameter1 == null ? null : sftp.proxies.Configuration.initialize(getContext(), __ConfigurationParameter1);

		// BEGIN USER CODE
		try {
			int reply = 200;
			IContext context = this.getContext();
			boolean SFTPused = false;
			FTPType type = ConfigurationParameter1.getFTPType(context) ; 
			if( type == FTPType.SFTP) {
				SFTPused = true;
			}
			if(SFTPused) {
				SFTPConnection.handleFile(DocumentParameter1, ConfigurationParameter1, ConnectionParameter1, null, context, Action.GET); 
			} else {
				reply = FTPConnection.execute(DocumentParameter1, ConfigurationParameter1, ConnectionParameter1, null, context, Action.GET);
			}
			return (long) reply;
		} catch (Exception e) {
			e.printStackTrace();
			Core.getLogger("(S)FTP").error(e);
			return (long) 0;
		}	
		// END USER CODE
	}

	/**
	 * Returns a string representation of this action
	 */
	@Override
	public String toString()
	{
		return "JA_GetDocument";
	}

	// BEGIN EXTRA CODE
	// END EXTRA CODE
}
