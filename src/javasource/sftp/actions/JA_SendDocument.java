...
/**
 * Tries to upload a document to the currently opened folder.
 * 
 * Document = A FileDocument entity which content will be uploaded
 * Connection = The user specific Connection entity
 * Configuration = The general Configuration entity
 */
public class JA_SendDocument extends CustomJavaAction<Long>
{
	...
	public JA_SendDocument(IContext context, IMendixObject DocumentParameter1, IMendixObject ConnectionParameter1, IMendixObject ConfigurationParameter1)
	{
		super(context);
	...
	public Long executeAction() throws Exception
	{
		...
		try {
			...
			String URL = ConnectionParameter1.getCurrentDirectory(context);
			...
			if(SFTPused) {
			SFTPConnection.handleFile(DocumentParameter1, ConfigurationParameter1, ConnectionParameter1, null, context, Action.PUT);
			} else {
			reply =	FTPConnection.execute(DocumentParameter1, ConfigurationParameter1, ConnectionParameter1, null, context, Action.PUT);
			}
			return (long) reply;
		} catch (Exception e) {
			e.printStackTrace();
